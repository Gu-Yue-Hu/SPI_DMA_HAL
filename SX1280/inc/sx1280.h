...
/*!
 * \brief 硬件IO IRQ回调函数定义
 */
typedef void ( DioIrqHandler )( void );

/*!
 * \brief 提供在无线电上运行的芯片的频率和频率阶跃
 *
 * \remark 这些定义用于计算分频器以设置RF频率
 */
#define XTAL_FREQ                                   52000000
#define FREQ_STEP                                   ( ( double )( XTAL_FREQ / pow( 2.0, 18.0 ) ) )

/*!
 * \brief SetAutoTx / Rx功能的补偿延迟（以微秒为单位）
 */
#define AUTO_RX_TX_OFFSET                           33

/*!
 * \brief 包含固件版本MSB的寄存器的地址
 */
#define REG_LR_FIRMWARE_VERSION_MSB                 0x0153

/*!
 * \brief 包含定义CRC种子的第一个字节的寄存器地址
 *
 * \remark 仅用于数据包类型GFSK和Flrc
 */
#define REG_LR_CRCSEEDBASEADDR                      0x09C8

/*!
 * \brief 包含定义CRC多项式的第一个字节的寄存器地址
 *
 * \remark 仅用于数据包类型GFSK和Flrc
 */
...
/*!
 * \brief 包含定义白化种子的第一个字节的寄存器地址
 *
 * \remark 仅用于数据包类型GFSK，FLRC和BLE
 */
#define REG_LR_WHITSEEDBASEADDR                     0x09C5

/*!
 * \brief 包含范围ID检查长度的寄存器的地址
 *
 * \remark 仅用于数据包类型测距 Ranging
 */
#define REG_LR_RANGINGIDCHECKLENGTH                 0x0931

/*!
 * \brief 存放设备范围ID的寄存器的地址--->设置设备测距地址
 *
 * \remark 仅用于数据包类型测距 Ranging
 */
...
/*!
 * \brief 存放设备范围ID的寄存器的地址-->设置测距请求地址
 *
 * \remark 仅用于数据包类型测距 Ranging
 */
...
/*!
 * \brief 存放量程结果的第一个字节的寄存器地址仅用于数据包类型测距
 */
#define REG_LR_RANGINGRESULTBASEADDR                0x0961

/*!
 * \brief 寄存器的地址允许读取测距结果
 *
 * \remark 仅用于数据包类型测距 Ranging
 */
...
/*!
 * \brief 内置测距滤波器考虑的默认样本数
 */
#define DEFAULT_RANGING_FILTER_SIZE                 127

/*!
 * \brief 存放LORA数据包参数的寄存器的地址
 */
#define REG_LR_PACKETPARAMS                         0x903

/*!
 * \brief 保持有效载荷长度的寄存器地址
 *
 * \remark 不要尝试直接阅读它。 请改用GetRxBuffer（）。
 */
...
typedef enum
{
    RADIO_PACKET_FIXED_LENGTH               = 0x00,         //!< 数据包在两侧都是已知的，数据包中不包含标头
    RADIO_PACKET_VARIABLE_LENGTH            = 0x20,         //!< 数据包大小可变，包含标头
...
/*!
 * \brief 持有LORA数据包类型的数据包长度模式
 */
typedef enum
{
    LORA_PACKET_VARIABLE_LENGTH             = 0x00,         //!< 数据包大小可变，包含标头  （显性标头）
    LORA_PACKET_FIXED_LENGTH                = 0x80,         //!< 数据包在两侧都是已知的，数据包中不包含标题 （隐式标头）
 ...
 * \brief 表示可用于收音机的中断屏蔽
 *
 * \remark 请注意，并非所有这些中断都适用于所有数据包类型
 */
typedef enum
{
...
/*!
 * \brief 描述每种数据包类型的调制参数的类型
 */
typedef struct
{
    RadioPacketTypes_t                    PacketType;        //调制参数所引用的数据包。
//    union
    struct
    {
        /*!
        ...
        struct
        {
            RadioLoRaSpreadingFactors_t  SpreadingFactor;   //LORA调制的扩展因子
            RadioLoRaBandwidths_t        Bandwidth;         //LORA调制的带宽
            RadioLoRaCodingRates_t       CodingRate;        //LORA调制的编码率
        }LoRa;
        /*!
        ...
        struct
        {
            RadioFlrcBitrates_t          BitrateBandwidth;  //!< FLRC调制的带宽和比特率值
            RadioFlrcCodingRates_t       CodingRate;        //!< FLRC调制的编码率
            RadioModShapings_t           ModulationShaping; //!< FLRC调制的调制整形
        }Flrc;
        /*!
...
typedef enum
{
    IRQ_HEADER_ERROR_CODE                   = 0x00,     //头错误
    IRQ_SYNCWORD_ERROR_CODE,                            //同步字错误
    IRQ_CRC_ERROR_CODE,                                 //CRC 错误
    IRQ_RANGING_ON_LORA_ERROR_CODE,                     //LORA 测距错误
}IrqErrorCode_t;
...
typedef struct
{
    void ( *txDone )( void );                       //!< 指向成功传输的函数的指针
    void ( *rxDone )( void );                       //!< 成功接收到的功能指针
    void ( *rxSyncWordDone )( void );               //!< 在成功接收SyncWord时运行的函数的指针
    void ( *rxHeaderDone )( void );                 //!< 成功接收标头后，指向该函数的指针
    void ( *txTimeout )( void );                    //!< 指向在传输超时时运行的功能的指针
    void ( *rxTimeout )( void );                    //!< Pointer to a function run on reception timeout
    void ( *rxError )( IrqErrorCode_t errCode );    //!< 指向接收错误时运行的功能的指针
...
//    union
    struct
    {
        ...
        struct
        {
            uint8_t                       PreambleLength;    //!< 前导码长度是前导码中的LORA符号的数量。 要进行设置，请使用以下公式 @code Number of symbols = PreambleLength[3:0] * ( 2^PreambleLength[7:4] ) @endcode
            RadioLoRaPacketLengthsModes_t HeaderType;        //!< 如果标头是显式的，它将在LORA数据包中传输。 如果标头是隐式的，则不会发送
            uint8_t                       PayloadLength;     //!< LORA数据包中有效负载的大小
            RadioLoRaCrcModes_t           CrcMode;           //!< LORA数据包中CRC块的大小
            RadioLoRaIQModes_t            InvertIQ;          //!< 允许将IQ交换为LORA数据包
        }LoRa;
        /*!
        ...
        struct
        {
            RadioPreambleLengths_t       PreambleLength;    //!< FLRC数据包类型的前导长度
            RadioFlrcSyncWordLengths_t   SyncWordLength;    //!< FLRC数据包类型的同步字长度
            RadioSyncWordRxMatchs_t      SyncWordMatch;     //!< 用于检查同步字的同步相关器
            RadioPacketLengthModes_t     HeaderType;        //!< 如果标头是显式的，它将在FLRC数据包中传输。 如果标头是隐式的，则不会发送。
            uint8_t                      PayloadLength;     //!< FLRC数据包中有效负载的大小
            RadioCrcTypes_t              CrcLength;         //!< FLRC数据包中CRC块的大小
            RadioWhiteningModes_t        Whitening;         //!< 如果报头类型可变，则在净荷和CRC块以及报头块上使用白化
        }Flrc;
        /*!
...
/*!
 * \brief 代表每种数据包类型的数据包状态
 */
typedef struct
{
    RadioPacketTypes_t                    packetType;        //!< 数据包状态所引用的数据包  数据包的类型是LORA/GFSK.....
    union
    {
        struct
        {
        ...
        struct
        {
            int8_t RssiPkt;                                 //!< 最后一个数据包的RSSI
            int8_t SnrPkt;                                  //!< 最后一个数据包的SNR
            struct                  //虽然声明自己是个bool(8bit)，但只占1bit的空间
            {
                bool SyncError :1;                          //!< SyncWord error on last packet
                bool LengthError :1;                        //!< Length error on last packet
                bool CrcError :1;                           //!< CRC error on last packet
            ...
            struct
            {
                bool RxNoAck :1;                            //!< 没有收到具有可变长度数据包的Rx的确认
                bool PacketSent :1;                         //!< 数据包已发送，仅与发送模式相关
            }TxRxStatus;                                    //!< Tx / Rx状态字节
            uint8_t SyncAddrStatus :3;                      //!< 找到数据包的相关器的ID
        }LoRa;
        struct
        {
            int8_t RssiAvg;                                 //!< 平均RSSI
            int8_t RssiSync;                                //!< 最后一个数据包的RSSI
            struct
            {
                bool SyncError :1;                          //!< SyncWord error on last packet
                bool LengthError :1;                        //!< Length error on last packet
